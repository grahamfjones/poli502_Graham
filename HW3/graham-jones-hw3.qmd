...
floor(x6)
ceiling(x6)
# The floor is a function that rounds the value down to the nearest integer.
# The ceiling is a function that rounds the value up to the nearest integer.
# To find out if your hunch was right, refer to the help file of these
# functions. Write a code to open up the help file for the floor function.
#WRITE YOUR ANSWER (code) HERE
help(floor)
...
manual_mean <- sum(vec.e) / length(vec.e)
builtin_mean <- mean(vec.e)
#In both cases, the same result occurs meaning that mean(x) is the same as sum(x) / length(x)
